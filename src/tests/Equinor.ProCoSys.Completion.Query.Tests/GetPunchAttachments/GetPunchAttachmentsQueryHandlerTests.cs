...
namespace Equinor.ProCoSys.Completion.Query.Tests.GetPunchAttachments;

[TestClass]
public class GetPunchAttachmentsQueryHandlerTests : TestsBase
{
    private GetPunchAttachmentsQueryHandler _dut;
    private Mock<IAttachmentService> _attachmentServiceMock;
    private GetPunchAttachmentsQuery _query;
    private AttachmentDto _attachmentDto;

    [TestInitialize]
    public void Setup()
    {
        _query = new GetPunchAttachmentsQuery(Guid.NewGuid());
        ...
        _attachmentServiceMock.Setup(l => l.GetAllForSourceAsync(_query.PunchGuid, default))
            .ReturnsAsync(attachmentDtos);

        _dut = new GetPunchAttachmentsQueryHandler(_attachmentServiceMock.Object);
    }
