...
public class NewAttachmentUploadedEventHandlerTests : EventHandlerTestBase
{
    ...
    public async Task Handle_ShouldPublish_CorrectIntegrationEvent()
    {
        ...
        Assert.AreEqual(_newAttachmentUploadedDomainEvent.Attachment.SourceGuid, _publishedIntegrationEvent.ParentGuid);
        Assert.AreEqual(_newAttachmentUploadedDomainEvent.Attachment.SourceType, _publishedIntegrationEvent.ParentType);
