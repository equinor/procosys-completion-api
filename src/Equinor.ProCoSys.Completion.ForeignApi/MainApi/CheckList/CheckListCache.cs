    ...
    : ICheckListCache
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var cachedChecklist = await distributedCache.GetStringAsync(checkListGuidCacheKey, token: cancellationToken);
        if (string.IsNullOrEmpty(cachedChecklist))
        {
            var checkList = await checkListApiService.GetCheckListAsync(checkListGuid);
            await distributedCache.SetStringAsync(checkListGuidCacheKey, JsonSerializer.Serialize(checkList), _options, token: cancellationToken);
            return checkList;
        }
